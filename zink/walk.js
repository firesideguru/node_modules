...
function diffDir (srcRoot, dstRoot, opts, path, diff) { // traverse() walk() diff(), do diff() via method calls, and test(), and copy() | del()
  
  if (!diff) {
    diff = { // make each of these methods
      ignoring: [],
      detached: [],
      notfound: [],
      modified: [],
  ...
  let srcList = listDir(srcRoot); // listSrcDir(srcRoot)
  let dstList = listDir(dstRoot); // listDstDir(dstRoot)
  ...
  srcList.forEach(srcItem => {
    ...
    let srcInfo = info(srcPath); // srcInfo, srcStat, srcCrc
    let dstInfo = info(dstPath); // dstInfo, dstStat, dstCrc
